...
<div class="tabla1">	
	...
	<tr>
		...
		<?php 
			...
			$sql="SELECT * FROM datos2 WHERE semestres = (select semestre from datos where email='$Email' and TipoRecomendacion = 'semestre')";
			$result=mysqli_query($conn,$sql);
		...
		<?php 
			...
			//$sql="SELECT * FROM datos2 WHERE semestres = (select semestre from datos where id=(SELECT MAX(id) AS id FROM datos));"
			$sql="SELECT * FROM datos2 WHERE /*(materia LIKE %check%) and*/ semestres = (select semestre from datos where email='$Email' and TipoRecomendacion = 'materia')";
			$result=mysqli_query($conn,$sql);
